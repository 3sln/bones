...
export default driver => {
  ...
  driver.panel('Demo', (container, signal) => {
    const app = div(
      ...
      p(
        'A numeric property is transformed by a pipeline. The color only changes when the number goes from even to odd, or vice-versa.',
      ),
      watch(number$, num => p(`Current number: ${num}`)),
      watch(color$, color =>
        p(
          {$styling: {color, fontWeight: 'bold'}},
          `The number is ${color === 'blue' ? 'even' : 'odd'}.`,
        ),
      ),
    );
